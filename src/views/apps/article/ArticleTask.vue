<!-- =========================================================================================
    ...
    Description: Single article item component
========================================================================================== -->
        ...
        <div class="vx-row">
            <div class="vx-col w-full sm:w-5/6 flex sm:items-center sm:flex-row flex-col">
                <div class="flex items-center">
                    ...
                    <h6 class="article-title" :class="{'line-through': taskLocal.isCompleted}">{{ taskLocal.title }}</h6>
                </div>
                <div class="article-tags sm:ml-2 sm:my-0 my-2 flex">
                    <vs-chip v-for="(tag, index) in taskLocal.tags" :key="index">
                        <div class="h-2 w-2 rounded-full mr-1" :class="'bg-' + articleLabelColor(tag)"></div>
...
export default{
  ...
  data () {
    return {
      taskLocal: this.$store.getters['article/getTask'](this.taskId)
    }
  },
  computed: {
    isCompleted: {
      ...
      set (value) {
        this.$store.dispatch('article/updateTask', Object.assign({}, this.taskLocal, {isCompleted: value}))
          .then((response) => {
    ...
    articleLabelColor () {
      return (label) => {
        const tags = this.$store.state.article.taskTags
  ...
  methods: {
    toggleIsImportant () {
      this.$store.dispatch('article/updateTask', Object.assign({}, this.taskLocal, {isImportant: !this.taskLocal.isImportant}))
        .then((response) => {
    ...
    toggleIsStarred () {
      this.$store.dispatch('article/updateTask', Object.assign({}, this.taskLocal, {isStarred: !this.taskLocal.isStarred}))
        .then((response) => {
    ...
    moveToTrash () {
      ...
      this.$store.dispatch('article/updateTask', Object.assign({}, this.taskLocal, {isTrashed: true}))
        .then((response) => {
