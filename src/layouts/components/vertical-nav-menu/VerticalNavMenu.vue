      ...
      <div @mouseenter="mouseEnter" @mouseleave="mouseLeave">
        ...
        <component :is="scrollbarTag" ref="verticalNavMenuPs" class="scroll-area-v-nav-menu pt-2" :settings="settings" @ps-scroll-y="psSectionScroll" @scroll="psSectionScroll" :key="$vs.rtl">
          <template v-for="(item, index) in menuItemsUpdated">
            ...
            <template v-else-if="item.isPopup">
              <v-nav-menu-item-popup :key="`item-${index}`"
                               :index="index"
                               :to="item.slug !== 'external' ? item.url : null"
                               :href="item.url"
                               :icon="item.icon" :target="item.target"
                               :isDisabled="item.isDisabled"
                               :formName="$t(item.i18n) || item.name"
                               :componentTemp="item.component"
                               :slug="item.slug">
...
import VNavMenuItemPopup from './VerticalNavMenuItemPopup.vue'
